...
/// This can be used to register an action with the NekoWG runtime when you want to manually implement
...
///
/// ```compile_fail
/// # #[macro_use] extern crate nekowg_macros;
/// # #[macro_use] extern crate nekowg;
/// #[derive(AppContext)]
/// struct MyContext<'a> {
///     app: &'a mut nekowg::App
/// }
/// ```
...
/// Used by NekoWG to generate the style helpers.
#[proc_macro]
#[doc(hidden)]
...
/// `#[nekowg::test]` can be used to annotate test functions that run with NekoWG support.
///
...
/// # Arguments
///
/// - `#[nekowg::test]` with no arguments runs once with the seed `0` or `SEED` env var if set.
/// - `#[nekowg::test(seed = 10)]` runs once with the seed `10`.
/// - `#[nekowg::test(seeds(10, 20, 30))]` runs three times with seeds `10`, `20`, and `30`.
/// - `#[nekowg::test(iterations = 5)]` runs five times, providing as seed the values in the range `0..5`.
/// - `#[nekowg::test(retries = 3)]` runs up to four times if it fails to try and make it pass.
/// - `#[nekowg::test(on_failure = "crate::test::report_failure")]` will call the specified function after the
///   tests fail so that you can write out more detail about the failure.
///
/// You can combine `iterations = ...` with `seeds(...)`:
/// - `#[nekowg::test(iterations = 5, seed = 10)]` is equivalent to `#[nekowg::test(seeds(0, 1, 2, 3, 4, 10))]`.
/// - `#[nekowg::test(iterations = 5, seeds(10, 20, 30)]` is equivalent to `#[nekowg::test(seeds(0, 1, 2, 3, 4, 10, 20, 30))]`.
/// - `#[nekowg::test(seeds(10, 20, 30), iterations = 5]` is equivalent to `#[nekowg::test(seeds(0, 1, 2, 3, 4, 10, 20, 30))]`.
///
...
/// A variant of `#[nekowg::test]` that supports property-based testing.
///
/// A property test, much like a standard NekoWG randomized test, allows testing
...
/// }
/// ```
/// Standard NekoWG randomized tests provide you with an instance of `StdRng` to
...
/// structure. In this case, a standard NekoWG randomized test may be more
/// suitable.
///
/// ## Customizing random values
///
/// This macro is based on the [`#[proptest::property_test]`] macro, but handles
/// some of the same NekoWG-specific arguments as `#[nekowg::test]`. Specifically,
...
/// `Arbitrary`. Parameters to a `#[nekowg::property_test]`, by default, use a
...
/// ## Scheduler
///
/// Similar to `#[nekowg::test]`, this macro will choose random seeds for the test
...
///
/// ```ignore
/// pub fn methods::<T: TheTrait + 'static>() -> Vec<nekowg::inspector_reflection::FunctionReflection<T>>;
///
/// pub fn find_method::<T: TheTrait + 'static>() -> Option<nekowg::inspector_reflection::FunctionReflection<T>>;
/// ```
///
