...
fn generate_reflected_trait(trait_item: ItemTrait) -> TokenStream {
    ...
    // Determine if we're being called from within the nekowg crate
    let call_site = Span::call_site();
    let inspector_reflection_path = if is_called_from_nekowg_crate(call_site) {
        quote! { crate::inspector_reflection }
    } else {
        quote! { ::nekowg::inspector_reflection }
    };
...
fn is_called_from_nekowg_crate(_span: Span) -> bool {
    // Check if we're being called from within the nekowg crate by examining the call site
    // This is a heuristic approach - we check if the current crate name is "nekowg"
    std::env::var("CARGO_PKG_NAME").is_ok_and(|name| name == "nekowg")
}
...
fn try_expand_macro(macro_item: &syn::TraitItemMacro) -> Option<Vec<TraitItem>> {
    ...
    match macro_name.as_str() {
        "nekowg_macros::style_helpers" | "style_helpers" => {
