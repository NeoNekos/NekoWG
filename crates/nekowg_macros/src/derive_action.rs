...
pub(crate) fn derive_action(input: TokenStream) -> TokenStream {
    ...
    */
    ...
    let build_fn_body = if no_json {
        ...
        quote! { Err(nekowg::private::anyhow::anyhow!(#error_msg)) }
    } else if is_unit_struct {
        quote! { Ok(Box::new(Self)) }
    } else {
        quote! { Ok(Box::new(nekowg::private::serde_json::from_value::<Self>(_value)?)) }
    };

    let json_schema_fn_body = if no_json || is_unit_struct {
        quote! { None }
    } else {
        quote! { Some(<Self as nekowg::private::schemars::JsonSchema>::json_schema(_generator)) }
    };
    ...
    TokenStream::from(quote! {
        ...
        impl nekowg::Action for #struct_name {
            ...
            fn partial_eq(&self, action: &dyn nekowg::Action) -> bool {
                action
                    .as_any()
                    .downcast_ref::<Self>()
            ...
            fn build(_value: nekowg::private::serde_json::Value) -> nekowg::Result<Box<dyn nekowg::Action>> {
                #build_fn_body
            }

            fn action_json_schema(
                _generator: &mut nekowg::private::schemars::SchemaGenerator,
            ) -> Option<nekowg::private::schemars::Schema> {
                #json_schema_fn_body
            }
