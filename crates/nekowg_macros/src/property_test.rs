...
pub fn test(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let Ok(func) = parse2::<ItemFn>(item) else {
        return quote_spanned! { item_span =>
            compile_error!("#[nekowg::property_test] must be placed on a function");
        };
    };
    ...
    let run_test_body = match &asyncness {
        ...
        Some(_) => quote! {
            let foreground_executor = nekowg::ForegroundExecutor::new(std::sync::Arc::new(dispatcher.clone()));
            #cx_vars
    ...
    quote! {
        ...
        #[::nekowg::proptest::property_test(proptest_path = "::nekowg::proptest", #args)]
        fn #test_name(#proptest_args) {
            #inner_fn

            ::nekowg::run_test_once(
                __seed,
...
fn remove_cxs(parsed: &mut ParsedArgs, args: &mut Vec<FnArg>, test_name: &Ident) {
    ...
    args.retain_mut(|arg| {
        ...
        parsed.cx_vars.extend(quote!(
            let mut #cx_varname = nekowg::TestAppContext::build(
                dispatcher.clone(),
...
fn remove_background_executor(parsed: &mut ParsedArgs, args: &mut Vec<FnArg>) {
    args.retain_mut(|arg| {
        ...
        parsed
            .inner_fn_args
            .extend(quote!(nekowg::BackgroundExecutor::new(
