//! Convenience crate that re-exports NekoWG's platform traits and the
//! `current_platform` constructor so consumers don't need `#[cfg]` gating.

pub use nekowg::Platform;

use std::rc::Rc;

/// Returns a background executor for the current platform.
pub fn background_executor() -> nekowg::BackgroundExecutor {
    current_platform(true).background_executor()
}

pub fn application() -> nekowg::Application {
    nekowg::Application::with_platform(current_platform(false))
}

pub fn headless() -> nekowg::Application {
    nekowg::Application::with_platform(current_platform(true))
}

/// Unlike `application`, this function returns a single-threaded web application.
#[cfg(target_family = "wasm")]
pub fn single_threaded_web() -> nekowg::Application {
    nekowg::Application::with_platform(Rc::new(nekowg_web::WebPlatform::new(false)))
}
...
pub fn current_platform(headless: bool) -> Rc<dyn Platform> {
    #[cfg(target_os = "macos")]
    {
        Rc::new(nekowg_macos::MacPlatform::new(headless))
    }

    #[cfg(target_os = "windows")]
    {
        Rc::new(
            nekowg_windows::WindowsPlatform::new(headless)
    ...
    {
        let _ = headless;
        Rc::new(nekowg_web::WebPlatform::new(true))
    }
}
...
mod tests {
    use super::*;
    use nekowg::{AppContext, Empty, VisualTestAppContext};
    ...
    // cargo test -p nekowg visual_test_context -- --ignored --test-threads=1
