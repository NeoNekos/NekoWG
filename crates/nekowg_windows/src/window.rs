...
impl WindowsWindowState {
    ...
    ///
    /// Currently, NekoWG uses the logical size of the app to handle mouse interactions (such as
    /// whether the mouse collides with other elements of NekoWG).
...
impl PlatformWindow for WindowsWindow {
    ...
    fn resize(&mut self, size: Size<Pixels>) {
        ...
        let bounds =
            nekowg::bounds(self.bounds().origin, size).to_device_pixels(self.scale_factor());
...
mod tests {
    ...
    use nekowg::{DevicePixels, MouseButton, point};
