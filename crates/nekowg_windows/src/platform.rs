...
impl WindowsPlatform {
    pub fn new(headless: bool) -> Result<Self> {
        ...
        let (directx_devices, text_system, direct_write_text_system) = if !headless {
            ...
            (
                None,
                Arc::new(nekowg::NoopTextSystem::new()) as Arc<dyn PlatformTextSystem>,
                None,
            )
        };
...
impl Platform for WindowsPlatform {
    ...
    fn screen_capture_sources(
        ...
        nekowg::scap_screen_capture::scap_screen_sources(&self.foreground_executor)
    }
    ...
    fn set_cursor_style(&self, style: CursorStyle) {
        ...
        if self.inner.state.current_cursor.get().map(|c| c.0) != hcursor.map(|c| c.0) {
            self.post_message(
                WM_NEKOWG_CURSOR_STYLE_CHANGED,
                WPARAM(0),
...
impl WindowsPlatformInner {
    ...
    fn handle_msg(
        ...
        let handled = match msg {
            WM_NEKOWG_CLOSE_ONE_WINDOW
            | WM_NEKOWG_TASK_DISPATCHED_ON_MAIN_THREAD
            | WM_NEKOWG_DOCK_MENU_ACTION
            | WM_NEKOWG_KEYBOARD_LAYOUT_CHANGED
            | WM_NEKOWG_GPU_DEVICE_LOST => self.handle_nekowg_events(msg, wparam, lparam),
            _ => None,
        };
    ...
    fn handle_nekowg_events(&self, message: u32, wparam: WPARAM, lparam: LPARAM) -> Option<isize> {
        ...
        match message {
            ...
            WM_NEKOWG_TASK_DISPATCHED_ON_MAIN_THREAD => self.run_foreground_task(),
            WM_NEKOWG_DOCK_MENU_ACTION => self.handle_dock_action_event(lparam.0 as _),
            WM_NEKOWG_KEYBOARD_LAYOUT_CHANGED => self.handle_keyboard_layout_change(),
            WM_NEKOWG_GPU_DEVICE_LOST => self.handle_device_lost(lparam),
            _ => unreachable!(),
        }
    }
        ...
        'tasks: loop {
            'timeout_loop: loop {
                if start.elapsed().as_millis() >= MAIN_TASK_TIMEOUT {
                    ...
                    // we spent our budget on nekowg tasks, we likely have a lot of work queued so drain system events first to stay responsive
                    // then quit out of foreground work to allow us to process other nekowg events first before returning back to foreground task work
                    ...
                    // Allow the main loop to process other nekowg events before going back into `run_foreground_task`
                    unsafe {
