...
static CLIPBOARD_HASH_FORMAT: LazyLock<u32> =
    LazyLock::new(|| register_clipboard_format(windows::core::w!("NekoWG internal text hash")));
static CLIPBOARD_METADATA_FORMAT: LazyLock<u32> =
    LazyLock::new(|| register_clipboard_format(windows::core::w!("NekoWG internal metadata")));
...
fn convert_image_to_png_format(bytes: &[u8], image_format: ImageFormat) -> Result<Vec<u8>> {
    let image =
        image::load_from_memory_with_format(bytes, nekowg_image_format_to_image(image_format))?;
...
fn nekowg_image_format_to_image(value: ImageFormat) -> image::ImageFormat {
    match value {
