...
mod macos_build {
    ...
    fn generate_shader_bindings() -> PathBuf {
        ...
        let nekowg_dir = find_nekowg_crate_dir();
        ...
        // Source files from nekowg that define types used in shaders
        let nekowg_src_paths = [
            nekowg_dir.join("src/scene.rs"),
            nekowg_dir.join("src/geometry.rs"),
            nekowg_dir.join("src/color.rs"),
            nekowg_dir.join("src/window.rs"),
            nekowg_dir.join("src/platform.rs"),
        ];

        // Source files from this crate
        let local_src_paths = [crate_dir.join("src/metal_renderer.rs")];

        for src_path in nekowg_src_paths.iter().chain(local_src_paths.iter()) {
    ...
    /// Locate the nekowg crate directory relative to this crate.
    fn find_nekowg_crate_dir() -> PathBuf {
        nekowg::NEKOWG_MANIFEST_DIR.into()
    }
