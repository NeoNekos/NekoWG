...
unsafe fn build_classes() {
    unsafe {
        APP_CLASS = {
            let mut decl = ClassDecl::new("NekoWGApplication", class!(NSApplication)).unwrap();
    ...
    unsafe {
        APP_DELEGATE_CLASS = unsafe {
            let mut decl =
                ClassDecl::new("NekoWGApplicationDelegate", class!(NSResponder)).unwrap();
            decl.add_ivar::<*mut c_void>(MAC_PLATFORM_IVAR);
            decl.add_method(
...
impl MacPlatform {
    pub fn new(headless: bool) -> Self {
        ...
        let text_system = Arc::new(nekowg::NoopTextSystem::new());
            ...
            match item {
                ...
                MenuItem::Action {
                    ...
                    let selector = match os_action {
                        Some(nekowg::OsAction::Cut) => selector("cut:"),
                        Some(nekowg::OsAction::Copy) => selector("copy:"),
                        Some(nekowg::OsAction::Paste) => selector("paste:"),
                        Some(nekowg::OsAction::SelectAll) => selector("selectAll:"),
                        // "undo:" and "redo:" are always disabled in our case, as
                        // we don't have a NSTextView/NSTextField to enable them on.
                        Some(nekowg::OsAction::Undo) => selector("handleNekoWGMenuItem:"),
                        Some(nekowg::OsAction::Redo) => selector("handleNekoWGMenuItem:"),
                        None => selector("handleNekoWGMenuItem:"),
                    };
...
impl Platform for MacPlatform {
    ...
    fn foreground_executor(&self) -> nekowg::ForegroundExecutor {
    ...
    fn screen_capture_sources(
        &self,
    ) -> oneshot::Receiver<Result<Vec<Rc<dyn nekowg::ScreenCaptureSource>>>> {
