...
unsafe fn build_classes() {
    unsafe {
        WINDOW_CLASS = build_window_class("NekoWGWindow", class!(NSWindow));
        PANEL_CLASS = build_window_class("NekoWGPanel", class!(NSPanel));
        VIEW_CLASS = {
            let mut decl = ClassDecl::new("NekoWGView", class!(NSView)).unwrap();
...
struct MacWindowState {
    ...
    event_callback: Option<Box<dyn FnMut(PlatformInput) -> nekowg::DispatchEventResult>>,
...
impl MacWindowState {
    ...
    fn bounds(&self) -> Bounds<Pixels> {
        ...
        if screen == nil {
            return Bounds::new(point(px(0.), px(0.)), nekowg::DEFAULT_WINDOW_SIZE);
        }
...
impl PlatformWindow for MacWindow {
    ...
    fn on_input(&self, callback: Box<dyn FnMut(PlatformInput) -> nekowg::DispatchEventResult>) {
    ...
    #[cfg(any(test, feature = "test-support"))]
    fn render_to_image(&self, scene: &nekowg::Scene) -> Result<RgbaImage> {
        let mut this = self.0.lock();
...
extern "C" fn handle_key_event(this: &Object, native_event: id, key_equivalent: bool) -> BOOL {
    ...
    match event {
        PlatformInput::KeyDown(key_down_event) => {
            ...
            // If that event isn't handled, it will then dispatch a "key down" event. NekoWG
...
extern "C" fn character_index_for_point(this: &Object, _: Sel, position: NSPoint) -> u64 {
    let position = screen_point_to_nekowg_point(this, position);
    with_input_handler(this, |input_handler| {
...
fn screen_point_to_nekowg_point(this: &Object, position: NSPoint) -> Point<Pixels> {
    let frame = get_frame(this);
