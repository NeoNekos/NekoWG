...
impl MetalGpuSurfaceState {
    ...
    fn sync_buffers(
        ...
        for (&handle, desc) in buffers {
            ...
            if needs_recreate {
                self.buffers
                    .insert(handle, create_gpu_surface_buffer(device, desc, is_unified_memory)?);
            }
        }
        Ok(())
    }
    ...
    fn execute_render_pass(
        ...
        let pipeline = self
            .render_pipeline(device, pass.program, gpu_texture_format_to_metal(target_desc.format))?
            .clone();
        let bindings = self
            .render_programs
            .get(&pass.program)
    ...
    fn bind_render_resources(
        ...
        for (binding, program_binding) in pass_bindings.iter().zip(program_bindings) {
            match (*binding, program_binding.kind) {
                (GpuBinding::UniformBuffer(handle), MetalGpuSurfaceProgramBindingKind::UniformBuffer)
                | (GpuBinding::StorageBuffer(handle), MetalGpuSurfaceProgramBindingKind::StorageBuffer) => {
                    let buffer = self
                        .buffers
                        .get(&handle)
                        .with_context(|| format!(
                ...
                (GpuBinding::SampledTexture(handle), MetalGpuSurfaceProgramBindingKind::SampledTexture)
                | (GpuBinding::StorageTexture(handle), MetalGpuSurfaceProgramBindingKind::StorageTexture) => {
                    let texture = self
                        .textures
                        .get(&handle)
                        .with_context(|| format!(
                ...
                (GpuBinding::Sampler(handle), MetalGpuSurfaceProgramBindingKind::Sampler) => {
                    ...
                    encoder
                        .set_vertex_sampler_state(program_binding.slot as u64, Some(&sampler.sampler));
...
fn build_gpu_surface_msl_source(
    ...
    let (source, _) = naga::back::msl::write_string(module, module_info, &options, &pipeline_options)
