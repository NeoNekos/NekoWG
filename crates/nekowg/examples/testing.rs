//! Example demonstrating NekoWG's testing infrastructure.
//!
//! When run normally, this displays an interactive counter window.
//! The tests below demonstrate various NekoWG testing patterns.
//!
//! Run the app: cargo run -p nekowg --example testing
//! Run tests:   cargo test -p nekowg --example testing --features test-support
...
struct Counter {
    ...
    _subscription: nekowg::Subscription,
}

/// Event emitted by Counter
struct CounterEvent;

impl nekowg::EventEmitter<CounterEvent> for Counter {}
...
fn run_example() {
    application().run(|cx: &mut App| {
        cx.bind_keys([
            nekowg::KeyBinding::new("up", Increment, Some("Counter")),
            nekowg::KeyBinding::new("down", Decrement, Some("Counter")),
        ]);
...
mod tests {
    use super::*;
    use nekowg::{TestAppContext, VisualTestContext};
    use rand::prelude::*;

    /// Here's a basic NekoWG test. Just add the macro and take a TestAppContext as an argument!
    ///
    ...
    /// NekoWG tests can also be async, simply add the async keyword before the test.
    ...
    /// something outside NekoWG's control, like a reading a file or network IO.
    ...
    async fn test_allow_parking(cx: &mut TestAppContext) {
        ...
        // Without allow_parking(), this await would panic because NekoWG's
        // scheduler runs out of tasks while waiting for the external thread.
        let result = rx.await.unwrap();
        assert_eq!(result, 42);
    }

    /// NekoWG also provides support for property testing, via the iterations flag
    #[nekowg::test(iterations = 10)]
    ...
    /// Now, all of those tests are good, but NekoWG also provides strong support for testing distributed systems.
