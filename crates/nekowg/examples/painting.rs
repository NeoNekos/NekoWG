...
impl PaintingViewer {
    fn new(_window: &mut Window, _cx: &mut Context<Self>) -> Self {
        ...
        builder.scale(0.9);
        let path = builder.build().unwrap();
        lines.push((path, nekowg::black().into()));
        ...
        lines.push((
            ...
            linear_gradient(
                180.,
                linear_color_stop(nekowg::blue(), 0.4),
                linear_color_stop(nekowg::red(), 1.),
            ),
        ));
        ...
        let path = builder.build().unwrap();
        lines.push((path, nekowg::green().into()));
        ...
        div()
            ...
            .child(
                div()
                            ...
                            move |_, _, window, _| {
                                ...
                                for points in lines {
                                    ...
                                    if let Ok(path) = builder.build() {
                                        window.paint_path(path, nekowg::black());
                                    }
                                }
                            },
                        )
                    ...
                    .on_mouse_move(cx.listener(|this, ev: &nekowg::MouseMoveEvent, _, cx| {
                        if !this._painting {
                            return;
                        }
