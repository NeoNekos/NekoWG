...
impl ImageCacheProvider for SimpleLruCacheProvider {
    fn provide(&mut self, window: &mut Window, cx: &mut App) -> nekowg::AnyImageCache {
        window
...
struct SimpleLruCache {
    max_items: usize,
    usages: Vec<u64>,
    cache: HashMap<u64, nekowg::ImageCacheItem>,
}
...
impl ImageCache for SimpleLruCache {
    fn load(
        &mut self,
        resource: &nekowg::Resource,
        window: &mut Window,
        cx: &mut App,
    ) -> Option<Result<Arc<nekowg::RenderImage>, nekowg::ImageCacheError>> {
        ...
        self.cache
            .insert(hash, nekowg::ImageCacheItem::Loading(task.clone()));
        self.usages.insert(0, hash);
