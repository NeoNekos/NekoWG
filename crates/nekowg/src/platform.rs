...
use crate::{
    ...
    ForegroundExecutor, GlyphId, GpuSpecs, GpuSurfaceExecutionInput, ImageSource, Keymap,
    LineLayout, PaintSurface, Pixels, PlatformInput, Point, Priority, RenderGlyphParams,
    RenderImage, RenderImageParams, RenderImageSource, RenderSvgParams, Scene, ShapedGlyph,
    ...
    fn screen_capture_sources(
        ...
        sources_tx
            .send(Err(anyhow::anyhow!(
                "nekowg was compiled without the screen-capture feature"
            )))
            .ok();
        sources_rx
    }
...
#[doc(hidden)]
pub type TimerResolutionGuard = nekowg_util::Deferred<Box<dyn FnOnce() + Send>>;
...
impl Image {
    ...
    /// Use the NekoWG `use_asset` API to make this image renderable
    ...
    /// Use the NekoWG `get_asset` API to make this image renderable
    ...
    /// Use the NekoWG `remove_asset` API to drop this image, if possible.
