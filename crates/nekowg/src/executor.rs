...
impl BackgroundExecutor {
    ...
    pub fn allow_parking(&self) {
        ...
        if std::env::var("NEKOWG_RUN_UNTIL_PARKED_LOG").ok().as_deref() == Some("1") {
            log::warn!("[nekowg::executor] allow_parking: enabled");
        }
    }
...
impl ForegroundExecutor {
    ...
    pub fn block_test<R>(&self, future: impl Future<Output = R>) -> R {
        ...
        // In async NekoWG tests, we must allow foreground tasks scheduled by the test itself
