//! Test support for NekoWG.
//!
//! NekoWG provides first-class support for testing, which includes a macro to run test that rely on having a context,
...
//!
//! The output of the `nekowg::test` macro is understood by other rust test runners, so you can use it with `cargo test`
...
/// Strategy injected into `#[nekowg::property_test]` tests to control the seed
...
/// `nekowg::property_test` macro and generally should not be used directly.
///
...
/// This is intended for use with the `nekowg::test` macro
