use crate::{
    ...
    Entity, EventEmitter, Focusable, ForegroundExecutor, Global, NekowgBorrow, PromptButton,
...
impl AppContext for AsyncApp {
    ...
    fn as_mut<'a, T>(&'a mut self, _handle: &Entity<T>) -> NekowgBorrow<'a, T>
    where
        T: 'static,
    {
...
impl AsyncApp {
    ...
    pub fn on_drop<T: 'static, Callback: FnOnce(&mut T, &mut Context<T>) + 'static>(
        ...
        entity: &WeakEntity<T>,
        f: Callback,
    ) -> nekowg_util::Deferred<impl FnOnce() + use<T, Callback>> {
...
impl AppContext for AsyncWindowContext {
    ...
    fn as_mut<'a, T>(&'a mut self, _: &Entity<T>) -> NekowgBorrow<'a, T>
    where
        T: 'static,
    {
