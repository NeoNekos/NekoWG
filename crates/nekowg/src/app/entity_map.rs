use crate::{App, AppContext, NekowgBorrow, VisualContext, Window, seal::Sealed};
...
impl<T: 'static> Entity<T> {
    ...
    /// Updates the entity referenced by this handle with the given function.
    #[inline]
    pub fn as_mut<'a, C: AppContext>(&self, cx: &'a mut C) -> NekowgBorrow<'a, T> {
        cx.as_mut(self)
    }
...
impl LeakDetector {
    ...
    pub fn handle_created(
        ...
        let id = nekowg_util::post_inc(&mut self.next_handle_id);
