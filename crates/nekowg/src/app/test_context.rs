use crate::{
    ...
    WindowHandle, WindowOptions, app::NekowgMode, window::ElementArenaScope,
};
...
/// A TestAppContext is provided to tests created with `#[nekowg::test]`, it provides
...
impl AppContext for TestAppContext {
    ...
    fn as_mut<'a, T>(&'a mut self, _: &Entity<T>) -> super::NekowgBorrow<'a, T>
    where
        T: 'static,
    {
...
impl TestAppContext {
    /// Creates a new `TestAppContext`. Usually you can rely on `#[nekowg::test]` to do this for you.
    pub fn build(dispatcher: TestDispatcher, fn_name: Option<&'static str>) -> Self {
        ...
        app.borrow_mut().mode = NekowgMode::test();
    ...
    pub fn skip_drawing(&mut self) {
        self.app.borrow_mut().mode = NekowgMode::Test { skip_drawing: true };
    }
...
impl AppContext for VisualTestContext {
    ...
    fn as_mut<'a, T>(&'a mut self, handle: &Entity<T>) -> super::NekowgBorrow<'a, T>
    where
        T: 'static,
    {
