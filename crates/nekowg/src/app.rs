...
use nekowg_util::{ResultExt, debug_panic};
...
/// A reference to a NekoWG application, typically constructed in the `main` function of your app.
...
impl NekowgMode {
    ...
    pub(crate) fn skip_drawing(&self) -> bool {
        match self {
            #[cfg(any(test, feature = "test-support"))]
            NekowgMode::Test { skip_drawing } => *skip_drawing,
            NekowgMode::Production => false,
        }
    }
}
...
impl App {
    ...
    /// Set the prompt renderer for NekoWG. This will replace the default or platform specific
    ...
    /// Tell NekoWG that an entity has changed and observers of it should be notified.
    ...
    /// Initializes nekowg's default colors for the application.
    ///
...
impl AppContext for App {
    ...
    fn as_mut<'a, T>(&'a mut self, handle: &Entity<T>) -> NekowgBorrow<'a, T>
    where
        T: 'static,
    {
        NekowgBorrow::new(handle.clone(), self)
    }
...
/// A mutable reference to an entity owned by NekoWG
pub struct NekowgBorrow<'a, T> {
    inner: Option<Lease<T>>,
    app: &'a mut App,
}

impl<'a, T: 'static> NekowgBorrow<'a, T> {
...
impl<'a, T: 'static> std::borrow::Borrow<T> for NekowgBorrow<'a, T> {
    fn borrow(&self) -> &T {
        self.inner.as_ref().unwrap().borrow()
    }
}

impl<'a, T: 'static> std::borrow::BorrowMut<T> for NekowgBorrow<'a, T> {
    fn borrow_mut(&mut self) -> &mut T {
        self.inner.as_mut().unwrap().borrow_mut()
    }
}

impl<'a, T: 'static> std::ops::Deref for NekowgBorrow<'a, T> {
    type Target = T;
...
impl<'a, T: 'static> std::ops::DerefMut for NekowgBorrow<'a, T> {
    fn deref_mut(&mut self) -> &mut T {
        self.inner.as_mut().unwrap()
    }
}

impl<'a, T> Drop for NekowgBorrow<'a, T> {
    fn drop(&mut self) {
