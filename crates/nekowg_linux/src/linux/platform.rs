    ...
    fn screen_capture_sources(
        ...
        sources_tx
            .send(Err(anyhow::anyhow!(
                "nekowg_linux was compiled without the screen-capture feature"
            )))
            .ok();
        sources_rx
    }
...
impl<P: LinuxClient + 'static> Platform for LinuxPlatform<P> {
    ...
    fn keyboard_mapper(&self) -> Rc<dyn PlatformKeyboardMapper> {
        Rc::new(nekowg::DummyKeyboardMapper)
    }
        ...
        self.foreground_executor()
            .spawn(async move {
                ...
                let request = match ashpd::desktop::file_chooser::OpenFileRequest::default()
                    ...
                    .modal(true)
                    .title(title)
                    .accept_label(options.prompt.as_ref().map(nekowg::SharedString::as_str))
        ...
        self.background_executor().spawn(async move {
            ...
            for item in items.into_iter() {
                if item.label().await.is_ok_and(|label| label == KEYRING_LABEL) {
                    ...
                    // a current limitation NekoWG's credentials api
...
pub(super) fn keystroke_from_xkb(
    state: &State,
    mut modifiers: nekowg::Modifiers,
    keycode: Keycode,
) -> nekowg::Keystroke {
...
#[cfg(any(feature = "wayland", feature = "x11"))]
pub(super) fn modifiers_from_xkb(keymap_state: &State) -> nekowg::Modifiers {
...
#[cfg(any(feature = "wayland", feature = "x11"))]
pub(super) fn capslock_from_xkb(keymap_state: &State) -> nekowg::Capslock {
...
#[cfg(any(feature = "wayland", feature = "x11"))]
pub(super) fn compositor_gpu_hint_from_dev_t(dev: u64) -> Option<nekowg_wgpu::CompositorGpuHint> {
    ...
    Some(nekowg_wgpu::CompositorGpuHint {
        vendor_id,
        device_id,
    })
}
