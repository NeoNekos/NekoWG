...
use http_client::Url;
use log::Level;
use nekowg::{Capslock, TaskTiming, profiler};
...
use nekowg_wgpu::{CompositorGpuHint, GpuContext};
...
const NEKOWG_X11_SCALE_FACTOR_ENV: &str = "NEKOWG_X11_SCALE_FACTOR";
    ...
    fn handle_event(&self, event: Event) -> Option<()> {
        match event {
            ...
            Event::SelectionNotify(event) => {
                ...
                if let Ok(file_list) = str::from_utf8(&reply.value) {
                    ...
                    let input = PlatformInput::FileDrop(FileDropEvent::Entered {
                        ...
                        paths: nekowg::ExternalPaths(paths),
                    });
                    drop(state);
            ...
            Event::KeyPress(event) => {
                ...
                drop(state);
                window.handle_input(PlatformInput::KeyDown(nekowg::KeyDownEvent {
                    keystroke,
                    is_held: false,
            ...
            Event::KeyRelease(event) => {
                ...
                drop(state);
                window.handle_input(PlatformInput::KeyUp(nekowg::KeyUpEvent { keystroke }));
            }
            Event::XinputButtonPress(event) => {
                ...
                match button_or_scroll_from_event_detail(event.detail) {
                    Some(ButtonOrScroll::Button(button)) => {
                        ...
                        drop(state);
                        window.handle_input(PlatformInput::MouseDown(nekowg::MouseDownEvent {
                            button,
                            position,
                            modifiers,
            ...
            Event::XinputButtonRelease(event) => {
                ...
                match button_or_scroll_from_event_detail(event.detail) {
                    Some(ButtonOrScroll::Button(button)) => {
                        ...
                        drop(state);
                        window.handle_input(PlatformInput::MouseUp(nekowg::MouseUpEvent {
                            button,
                            position,
                            modifiers,
                            click_count,
            ...
            Event::XinputMotion(event) => {
                ...
                if event.valuator_mask[0] & 3 != 0 {
                    window.handle_input(PlatformInput::MouseMove(nekowg::MouseMoveEvent {
                        position,
                        pressed_button,
                        modifiers,
                    }));
            ...
            Event::XinputLeave(event) if event.mode == xinput::NotifyMode::NORMAL => {
                ...
                let window = self.get_window(event.event)?;
                window.handle_input(PlatformInput::MouseExited(nekowg::MouseExitEvent {
                    pressed_button,
                    position,
                    modifiers,
                }));
...
impl LinuxClient for X11Client {
    ...
    fn screen_capture_sources(
        &self,
    ) -> futures::channel::oneshot::Receiver<anyhow::Result<Vec<Rc<dyn nekowg::ScreenCaptureSource>>>>
    {
        nekowg::scap_screen_capture::scap_screen_sources(
    ...
    fn write_to_primary(&self, item: nekowg::ClipboardItem) {
        let state = self.0.borrow_mut();
        state
            .clipboard
            .set_text(
    ...
    fn write_to_clipboard(&self, item: nekowg::ClipboardItem) {
        let mut state = self.0.borrow_mut();
        state
            .clipboard
            .set_text(
    ...
    fn read_from_primary(&self) -> Option<nekowg::ClipboardItem> {
        let state = self.0.borrow_mut();
        state
            .clipboard
    ...
    fn read_from_clipboard(&self) -> Option<nekowg::ClipboardItem> {
        let state = self.0.borrow_mut();
...
fn get_scale_factor(
    ...
    let env_dpi = std::env::var(NEKOWG_X11_SCALE_FACTOR_ENV)
        .ok()
        .map(|var| {
            if var.to_lowercase() == "randr" {
                ...
                if valid_scale_factor(scale) {
                    ...
                    panic!(
                        ...
                        NEKOWG_X11_SCALE_FACTOR_ENV, var
                    );
                }
