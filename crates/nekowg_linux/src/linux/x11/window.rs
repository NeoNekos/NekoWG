...
pub struct Callbacks {
    ...
    input: Option<Box<dyn FnMut(PlatformInput) -> nekowg::DispatchEventResult>>,
...
impl X11WindowState {
    pub fn new(
        ...
        client: X11ClientStatePtr,
        executor: ForegroundExecutor,
        gpu_context: nekowg_wgpu::GpuContext,
...
impl PlatformWindow for X11Window {
    ...
    fn capslock(&self) -> nekowg::Capslock {
        self.0
            .state
            .borrow()
            .client
    ...
    fn window_decorations(&self) -> nekowg::Decorations {
        let state = self.0.state.borrow();
    ...
    fn request_decorations(&self, mut decorations: nekowg::WindowDecorations) {
        let mut state = self.0.state.borrow_mut();

        if matches!(decorations, nekowg::WindowDecorations::Client)
            && !state.client_side_decorations_supported
        {
            log::info!(
                "x11: no compositor present, falling back to server-side window decorations"
            );
            decorations = nekowg::WindowDecorations::Server;
        }
