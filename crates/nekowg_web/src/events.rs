...
impl WebWindowInner {
    ...
    fn register_pointer_down(self: &Rc<Self>) -> Closure<dyn FnMut(JsValue)> {
        ...
        self.listen("pointerdown", move |event: JsValue| {
            ...
            let button = dom_mouse_button_to_nekowg(event.button());
    ...
    fn register_key_down(self: &Rc<Self>) -> Closure<dyn FnMut(JsValue)> {
        ...
        self.listen("keydown", move |event: JsValue| {
            ...
            let key = dom_key_to_nekowg_key(&event);
...
fn dom_key_to_nekowg_key(event: &web_sys::KeyboardEvent) -> String {
    let key = event.key();
    match key.as_str() {
...
fn dom_mouse_button_to_nekowg(button: i16) -> MouseButton {
    match button {
        0 => MouseButton::Left,
...
fn compute_key_char(
    ...
    if is_modifier_only_key(nekowg_key) {
        return None;
    }
